...
exports.forgotPassword = catchAsyncErrors(async (req,res, next) => {
    ...
    const message = `Your password reset token is as follows: \n\n${resetUrl}\n\n If you have not requested this emamil, then ignore it.`;

    try {
        await sendEmail({
            email: user.email,
            subject: 'MeroShop Password Recovery',
            message
        })
...
exports.resetPassword = catchAsyncErrors(async (req,res, next) => {
    //Hash URL token
    const resetPassswordToken = crypto.createHash('sha256').update(req.params.token).digest('hex');
